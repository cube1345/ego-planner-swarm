...
DURATION=20
STARTUP_WAIT=8
LABEL_PREFIX="adaptive_min_hits_eval"

usage() {
  cat <<'EOF'
Usage: bash tools/evaluate_adaptive_min_hits.sh [options]
...
echo "[adaptive_min_hits_eval] running OFF baseline..."
bash tools/compare_fusion.sh \
  --label "$OFF_LABEL" \
  --out-dir "$OUT_DIR" \
  --duration "$DURATION" \
  --startup-wait "$STARTUP_WAIT" \
  --adaptive-min-hits-enable False

echo "[adaptive_min_hits_eval] running ON baseline..."
bash tools/compare_fusion.sh \
  --label "$ON_LABEL" \
  --out-dir "$OUT_DIR" \
  --duration "$DURATION" \
  --startup-wait "$STARTUP_WAIT" \
  --adaptive-min-hits-enable True
