...
DURATION=24
STARTUP_WAIT=8
LABEL_PREFIX="adaptive_z_max_eval"
Z_MIN="3.0"
Z_MAX="3.5"
Z_STEP="0.25"
...
usage() {
  cat <<'EOF'
Usage: bash tools/evaluate_adaptive_z_max.sh [options]
...
echo "[adaptive_z_max_eval] running OFF baseline..."
bash tools/compare_fusion.sh \
  --label "$OFF_LABEL" \
  --out-dir "$OUT_DIR" \
  --duration "$DURATION" \
  --startup-wait "$STARTUP_WAIT" \
  --adaptive-z-max-enable False

echo "[adaptive_z_max_eval] running ON baseline..."
bash tools/compare_fusion.sh \
  --label "$ON_LABEL" \
  --out-dir "$OUT_DIR" \
  --duration "$DURATION" \
  --startup-wait "$STARTUP_WAIT" \
  --adaptive-z-max-enable True \
  --adaptive-z-max-min "$Z_MIN" \
  --adaptive-z-max-max "$Z_MAX" \
  --adaptive-z-max-step "$Z_STEP"
...
if (
    ...
    and delta["fusion_recall"] >= 0.0015
    and delta["fusion_f1_gain_vs_best_single"] >= -0.0005
    and delta["fusion_recall_gain_vs_best_single"] >= -0.0005
):
    verdict = "keep"
elif delta["fusion_f1"] <= -0.003 or delta["fusion_recall"] <= -0.0015:
    verdict = "disable"
else:
    verdict = "neutral"
