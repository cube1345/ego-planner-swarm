...
Options:
  ...
  --local-update-range-x X          Override local map update range X
  --local-update-range-y Y          Override local map update range Y
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --closed-loop-feedback)
      ...
      shift 2
      ;;
    --local-update-range-x)
      LOCAL_UPDATE_RANGE_X="$2"
      shift 2
      ;;
    --local-update-range-y)
      LOCAL_UPDATE_RANGE_Y="$2"
      shift 2
      ;;
    --rviz-config)
      RVIZ_CONFIG="$2"
...
cleanup_existing() {
  local pattern
  pattern='[s]ingle_run_in_sim_fusion.launch.py|[s]ingle_run_in_sim.launch.py|[e]go_planner_node|[t]raj_server|[p]oscmd_2_odom|[o]dom_visualization|[p]cl_render_node|[s]imulated_lidar_cloud.py|[r]os2_lidar_depth_fusion_node.py|[c]losed_loop_feedback_node.py|[d]ynamic_obstacle_cloud.py|[r]viz2'
...
if [[ "$MODE" == "fusion" ]]; then
  if [[ "$USE_DYNAMIC" =~ ^([Tt]rue|true|1|yes|YES)$ ]]; then
    LOCAL_UPDATE_RANGE_X="${LOCAL_UPDATE_RANGE_X:-6.5}"
    LOCAL_UPDATE_RANGE_Y="${LOCAL_UPDATE_RANGE_Y:-6.5}"
  fi
  LAUNCH_CMD=(
  ...
  if [[ -n "$LOCAL_UPDATE_RANGE_X" ]]; then
    LAUNCH_CMD+=(local_update_range_x:="$LOCAL_UPDATE_RANGE_X")
  fi
  if [[ -n "$LOCAL_UPDATE_RANGE_Y" ]]; then
    LAUNCH_CMD+=(local_update_range_y:="$LOCAL_UPDATE_RANGE_Y")
  fi
else
  LAUNCH_CMD=(
