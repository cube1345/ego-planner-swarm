...
DURATION=20
STARTUP_WAIT=8
LABEL_PREFIX="adaptive_lidar_growth_eval"
GROWTH_MIN="3.8"
GROWTH_MAX="4.2"
GROWTH_STEP="0.2"
...
usage() {
  cat <<'EOF'
Usage: bash tools/evaluate_adaptive_lidar_growth.sh [options]
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --growth-min) GROWTH_MIN="$2"; shift 2 ;;
    --growth-max) GROWTH_MAX="$2"; shift 2 ;;
    --growth-step) GROWTH_STEP="$2"; shift 2 ;;
    -h|--help) usage; exit 0 ;;
    *)
...
echo "[adaptive_lidar_growth_eval] running OFF baseline..."
bash tools/compare_fusion.sh \
  --label "$OFF_LABEL" \
  --out-dir "$OUT_DIR" \
  --duration "$DURATION" \
  --startup-wait "$STARTUP_WAIT" \
  --adaptive-lidar-growth-enable False

echo "[adaptive_lidar_growth_eval] running ON baseline..."
bash tools/compare_fusion.sh \
  --label "$ON_LABEL" \
  --out-dir "$OUT_DIR" \
  --duration "$DURATION" \
  --startup-wait "$STARTUP_WAIT" \
  --adaptive-lidar-growth-enable True \
  --adaptive-lidar-growth-min "$GROWTH_MIN" \
  --adaptive-lidar-growth-max "$GROWTH_MAX" \
  --adaptive-lidar-growth-step "$GROWTH_STEP"
