...
### 3.4 Dempster-Shafer 不确定性建模

在 log-odds 融合输出之外，当前系统新增了 Dempster-Shafer 证据指标层。对每个体素，depth 与 lidar 的概率证据会被映射为三类质量：

- 占据质量 $m(O)$
- 空闲质量 $m(F)$
- 未知质量 $m(U)$

两路传感器证据通过 D-S 组合规则进行合成，同时显式计算冲突项：

$$
K = m_d(O)m_l(F) + m_d(F)m_l(O)
$$

组合后的占据置信度、未知度和冲突度被聚合为在线诊断指标，并发布到：

```text
/drone_0_fusion/ds_metrics
```

当前 D-S 层不直接替换 `fused_cloud` 的筛选逻辑，而是作为融合质量解释和后续自适应优化的指标层。这样设计可以避免在未完成充分 A/B 验证前改变 planner 输入分布，同时为后续基于 `unknown_mean` 和 `conflict_mean` 的闭环优化保留接口。
...
加入 Dempster-Shafer 证据指标层后，系统可以额外观测融合输出的不确定性和传感器冲突度，但当前该层仍定位为诊断指标，不作为替代规划输入的主输出。
